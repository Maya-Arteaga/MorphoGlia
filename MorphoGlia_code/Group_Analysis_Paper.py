...
for category, color in cluster_colors.items():
    ...
    # Plot all other categories in the background
    other_categories = [cat for cat in cluster_colors.keys() if cat != category]
    for other_category in other_categories:
        other_category_data = data[data['categories'] == other_category]
        plt.scatter(
            embedding[other_category_data.index, 0],
            embedding[other_category_data.index, 1],
            color='gainsboro',
            alpha=0.8,
            label=other_category if other_category == other_categories[0] else ""
        )
    
    # Plot the category of interest on top
